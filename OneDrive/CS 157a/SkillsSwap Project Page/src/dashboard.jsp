...
<%
    ...
    try {
        ...
        while (rs.next()) {
            skillsOffered.add(rs.getString("Title"));
        }
        ...
        stmt = conn.prepareStatement(
            "SELECT COUNT(*) AS cnt FROM Notifications WHERE User_ID = ? AND Status = 'Unread'"
        );
        stmt.setInt(1, userId);
        rs = stmt.executeQuery();

        if (rs.next()) {
            unreadNotifications = rs.getInt("cnt");
        }
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .navbar {
            display: flex; align-items: center; gap: 16px; padding: 15px 25px;
            background: #F8C8DC; border-radius: 20px; margin-bottom: 30px;
        }
        .navbar .brand {
            font-family: cursive; font-size: 1.4rem; color: #7a2d3b;
            font-weight: 700; margin-right: auto;
        }
        .navbar a {
            text-decoration: none; color: #2c7be5; font-weight: bold;
            padding: 6px 12px; border-radius: 8px;
        }
        .navbar a:hover { background: rgba(255,255,255,0.6); }
        .navbar .btn-logout {
            background: #AA336A; color: white; padding: 7px 16px;
            border-radius: 10px;
        }

        .welcome-banner {
            background: linear-gradient(135deg, #D8BFD8 0%, #F8C8DC 100%);
            border-radius: 20px; padding: 30px 35px; margin-bottom: 30px;
            display: flex; align-items: center; gap: 24px;
        }
        .avatar {
            width: 70px; height: 70px; border-radius: 50%; background: #AA336A;
            color: white; font-family: cursive; font-size: 1.8rem;
            display: flex; align-items: center; justify-content: center;
        }
        .welcome-text h2 {
            font-family: cursive; font-size: 1.7rem; color: #7a2d3b; margin-bottom: 4px;
        }
        .welcome-text p { color: #666; font-size: 0.95rem; }
        .role-badge {
            display: inline-block; background: #AA336A; color: white;
        ...
        .stat-card {
            flex: 1; min-width: 150px; background: #fff; border: 2px solid #F3CFC6;
            border-radius: 16px; padding: 22px 24px; text-align: center;
            box-shadow: 0 2px 10px rgba(170,51,106,0.06);
        }
        .stat-value {
            font-family: cursive; font-size: 2.2rem; color: #AA336A; font-weight: 700;
        }
        .stat-label { color: #888; font-size: 0.85rem; margin-top: 4px; }

        .section-title {
            font-family: cursive; font-size: 1.4rem; color: #7a2d3b; margin-bottom: 18px;
        }
        .actions-grid {
        ...
        .action-card {
            background: #fff; border: 2px solid #F3CFC6; border-radius: 16px;
        ...
        .action-card:hover {
            transform: translateY(-3px); box-shadow: 0 6px 20px rgba(170,51,106,0.13);
            border-color: #AA336A;
        }
        .action-icon { font-size: 2rem; margin-bottom: 10px; }
        .action-label { font-weight: bold; color: #7a2d3b; font-size: 0.95rem; }
        .action-desc { color: #999; font-size: 0.82rem; margin-top: 4px; }

        .info-card {
            background: #fff; border: 2px solid #F3CFC6; border-radius: 16px;
            padding: 28px 30px; box-shadow: 0 2px 10px rgba(170,51,106,0.06);
        }
        .info-row {
            display: flex; gap: 12px; padding: 10px 0;
            border-bottom: 1px solid #f9e8f0; font-size: 0.95rem;
        }
...
<nav class="navbar">
    ...
    <a href="<%= request.getContextPath() %>/src/notifications.jsp">
        Notifications<% if (unreadNotifications > 0) { %> (<%= unreadNotifications %>)<% } %>
    </a>
...
<div class="stats-row">
    ...
    <div class="stat-card">
        <%
            String ratingDisplay = reviewCount == 0 ? "—" : String.format("%.1f", avgRating);
        %>
...
<div class="actions-grid">
    ...
    <a class="action-card" href="<%= request.getContextPath() %>/src/notifications.jsp">
        <div class="action-icon">&#128276;</div>
        <div class="action-label">
            Notifications<% if (unreadNotifications > 0) { %> (<%= unreadNotifications %>)<% } %>
        </div>
        <div class="action-desc">View alerts for messages and exchanges</div>
    </a>
    ...
    <a class="action-card" href="<%= request.getContextPath() %>/src/adminDashboard.jsp">
        <div class="action-icon">🛠️</div>
        <div class="action-label">Admin Panel</div>
        <div class="action-desc">Manage users, reviews, and system activity</div>
    </a>
    <% } %>
...
<div class="info-card">
    ...
    <div class="info-row"><span class="info-key">Full Name</span><span class="info-val"><%= userName %></span></div>
    <div class="info-row"><span class="info-key">Major</span><span class="info-val"><%= major.isEmpty() ? "—" : major %></span></div>
    <div class="info-row"><span class="info-key">Bio</span><span class="info-val"><%= bio.isEmpty() ? "—" : bio %></span></div>
    <div class="info-row"><span class="info-key">Availability</span><span class="info-val"><%= availability.isEmpty() ? "—" : availability %></span></div>
    ...
    <div class="info-row"><span class="info-key">Skills I Want</span><span class="info-val"><%= skillsWanted.isEmpty() ? "—" : skillsWanted %></span></div>
    <div class="info-row"><span class="info-key">Role</span><span class="info-val"><%= role %></span></div>
</div>

<div style="margin-top: 24px; display: flex; gap: 14px; flex-wrap: wrap;">
    <a href="editProfile.jsp"
       style="padding: 10px 22px; background: #AA336A; color: white; border-radius: 12px; font-weight: bold; text-decoration: none;">
        &#9998; Edit Profile
    </a>
    <a href="deleteAccount.jsp"
       style="padding: 10px 22px; background: #fff; color: #c0392b; border: 2px solid #f5c6cb; border-radius: 12px; font-weight: bold; text-decoration: none;">
        &#128465; Delete Account
    </a>
</div>
